...
export default function TerminalHeaderBar() {
  const [state, dispatch]: any = useContext(MyContext);
  return (
    <div
      className={`main--terminal__container  mode__${
        state.mode ? "FULLSCREEN" : "REGULAR"
      }  rounded-xl sm:container sm:mx-auto`}
    >
      <div className="main--terminal__container-headerbar headerbar">
        <span
          className="headerbar--minmax flex items-center justify-center"
          onClick={() => {
            dispatch(TOGGLE_TERMINAL_MODE, {mode: !state.mode});
          }}
        >
          {!state.mode ? (
